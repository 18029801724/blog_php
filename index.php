...
<div id="list">
	<h2>帖子列表</h2>
			<form action="search.php" method="post" target="_blank" class="search">
        <input type="search" name="search" required autofocus placeholder="请输入关键词">
	...
	<ul class="article">
		<?php
			...
			while (!!$_rows = _fetch_array_list($_result)) {
				...
				echo '<li class="icon'.$_htmllist['type'].'"><em>阅读数(<strong>'.$_htmllist['readcount'].'</strong>) 评论数(<strong>'.$_htmllist['commendcount'].'</strong>)</em> <a href="article.php?id='.$_htmllist['id'].'">'._title($_htmllist['title'],27).'</a></li>';
			}
...
<div id="user">
	...
	<dl >
        		<?php
			$_htmllist = array();
			while (!!$_rows = _fetch_array_list($_result1)) {
				$_htmllist['id'] = $_rows['tg_id'];
				$_htmllist['type'] = $_rows['tg_type'];
				$_htmllist['title'] = $_rows['tg_title'];
				$_htmllist = _html($_htmllist);
				echo '<li><a href="article.php?id='.$_htmllist['id'].'">'._title($_htmllist['title'],22).'</a></li>';
			}
